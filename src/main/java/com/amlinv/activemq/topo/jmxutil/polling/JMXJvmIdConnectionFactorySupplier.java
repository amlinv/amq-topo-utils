 ...
 */
public class JMXJvmIdConnectionFactorySupplier implements MBeanAccessConnectionFactorySupplier {
    @Override
    public boolean handlesLocation(String location) {
        if ( location.startsWith("jvmId=") || location.startsWith("pid=") ) {
            return true;
        }
        return false;
    }
    ...
    public MBeanAccessConnectionFactory createFactory(String location) throws Exception {
        ...
        return new JMXJvmIdConnectionFactory(id);
    }
}
