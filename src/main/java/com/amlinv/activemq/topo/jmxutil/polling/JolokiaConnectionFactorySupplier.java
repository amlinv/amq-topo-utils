 ...
 */
public class JolokiaConnectionFactorySupplier implements MBeanAccessConnectionFactorySupplier {
    @Override
    public boolean handlesLocation(String location) {
        if ( location.startsWith("jolokia:") ) {
            return true;
        }
    ...
    public MBeanAccessConnectionFactory createFactory(String location) throws Exception {
        return new JolokiaConnectionFactory(location.replace("jolokia:", ""));
    }
}
